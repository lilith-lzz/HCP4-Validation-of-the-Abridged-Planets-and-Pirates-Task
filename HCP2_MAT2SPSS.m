...
RT_highcutoff = 30; % valid RT upper cut-off in secs (any RT/#qs > high cutoff = invalid RT)
 ...
 ques_names = {'audit_overall'}; % questionnaire names

% ques_names = {'audit_haz' 'audit_dep' 'audit_harm' ...
%   'audit_overall'}; % questionnaire names
...
SPSS_mat(ismember({HCP_aggr(:).Gender},'man'),4) = 1;
SPSS_mat(ismember({HCP_aggr(:).Gender},'woman'),4) = 2;
SPSS_mat(ismember({HCP_aggr(:).Gender},'non-binary'),4) = 3;
SPSS_mat(ismember({HCP_aggr(:).Gender},'prefer not to say'),4) = 4;

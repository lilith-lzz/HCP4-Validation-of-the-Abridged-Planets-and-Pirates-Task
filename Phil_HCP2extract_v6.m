...
pun_trials = 2:5; % reduced to 5 blocks in total 

end_trial_wait = 1000;
block_duration = 120*1000+end_trial_wait; % block duration reduced to 2mins

HCP_parameters.signal_time = 2000;
HCP_parameters.ship_duration = 6000; 
HCP_parameters.ship_attack_time = 4000; % OG = 6000ms
HCP_parameters.feedback_duration = 2000; % OG = 2500ms
HCP_parameters.shield_charging_time = 2000; % OG = 3000ms
...
ques_names = {'ques_audit'}; % questionnaire names
%ques_checks = {[7;0] [] [7;3]}; % [index;correct answer] for catch questions
RT_check_grid = [3 6 6 6 6; 1 4 4 4 4; 1 4 4 4 4; 0 1 1 1 1; 0 1 1 1 1]; % # post-block val/inf questions
...
work_folder = ['C:\Users\zzen0204\OneDrive - The University of Sydney (Staff)\Desktop\PhD\HCP+\Data Analyses & Graphing\HCP4 analyses\MATLAB'];
...
copy_file = {'Phil_HCP2extract_v6.m' 'AvBbinning.m' 'bin_rate2.m' 'HCP2_sumfig2.m' ...
...
%         '/' num2str(round(HCP_aggr(r).Block.ExpUnp_ShipRate(p),3))]);
  ...
  HCP_aggr(r).ValInf_AvgRT = vertcat(HCP_aggr(r).Block.ValCheckRT,...
    ...
    %HCP_aggr(r).Block.PredCheckRT, HCP_aggr(r).Block.RevalCheckRT)./RT_check_grid
  ...
  % Check if HCP_aggr(r).raw{q_idx,3} is a struct and contains 'responses'
  % Check if HCP_aggr(r).raw has a valid entry at {q_idx,3} and proceed if it exists
  ...
  elseif any(HCP_aggr(r).Catch) ...
    ...
    HCP_aggr(r).Exclude = false; %changed from true to false
  elseif any(HCP_aggr(r).Catch)
  ...
  %% Summary plot    
  sum_fig = HCP2_sumfig2(HCP_aggr(r).Block,pun_trials);
